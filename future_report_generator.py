...
from report_generator import get_llm_model, initialize_reports_db, save_report_to_db, load_reports_from_db, _postprocess_report_output
...
async def _get_serper_results_with_retry(serper_search_tool, query_string: str, company_name: str, num_results=10) -> List[Document]:
    """
                ...
                try:
                    ...
                    if documents and documents[0].page_content:
                        ...
                        metadata = {
                            ...
                            "company_name": company_name # 메타데이터에 company_name 추가
                        }
    ...
    if 'answerBox' in results and 'snippet' in results['answerBox']:
        ...
        metadata = {"source": results['answerBox'].get('link', 'N/A'), "title": results['answerBox'].get('title', 'N/A') or "Answer Box", "query_origin": query_string, "fetched_type": "answer_box_snippet", "company_name": company_name} # 메타데이터에 company_name 추가
        extracted_docs.append(Document(page_content=page_content, metadata=metadata))
    ...
    if 'knowledgeGraph' in results and 'snippet' in results['knowledgeGraph']:
        ...
        metadata = {"source": results['knowledgeGraph'].get('link', 'N/A'), "title": results['knowledgeGraph'].get('title', 'N/A') or "Knowledge Graph", "query_origin": query_string, "fetched_type": "knowledge_graph_snippet", "company_name": company_name} # 메타데이터에 company_name 추가
        extracted_docs.append(Document(page_content=page_content, metadata=metadata))

    return extracted_docs


async def _generate_page_4_future_report(query: str, progress_callback=None, perform_serper_search: bool = True) -> str:
    """
    ...
    try:
        ...
        existing_report_content = load_reports_from_db(
            report_type=report_type, company=query, year=current_year
        )
        ...
        if perform_serper_search:
            ...
            for i, search_q in enumerate(search_queries):
                ...
                try:
                    docs_for_query = await _get_serper_results_with_retry(serper_search, search_q, query) 
            ...
            if not raw_serper_docs_for_vectorstore:
                ...
                current_db_ids = set(vectorstore.get(where={"company_name": query}, include=['metadatas'])['ids'])
                ...
                if documents_to_add:
                    ...
                    for i in range(0, len(documents_to_add), batch_size):
                        ...
                        await asyncio.sleep(60) # 대량 추가 후 ChromaDB의 안정성을 위해 잠시 대기
        ...
        retriever = vectorstore.as_retriever(
            ...
            search_kwargs={
                "k": 20,
                "filter": {"company_name": query} # 특정 기업에 대한 문서만 가져오도록 필터링
            }
        )
        ...
        # 8. 생성된 보고서를 DB에 저장
        save_report_to_db( 
            report_type=report_type, company=query, year=current_year, month=None, content=roadmap_content
        )
...
if __name__ == '__main__':
    ...
    # 🚀 첫 번째 실행: Serper 검색 수행 (새로운 데이터 수집)
    print(f"\n--- 첫 번째 실행: '{test_query}' (Serper 검색 포함) ---")
    final_report_first_run = asyncio.run(_generate_page_4_future_report(test_query, perform_serper_search=True))
    print("\n--- 첫 번째 실행 최종 보고서 ---")
    print(final_report_first_run)

    # 🚀 두 번째 실행: Serper 검색 건너뛰기 (기존 DB에서 검색)
    print(f"\n--- 두 번째 실행: '{test_query}' (Serper 검색 건너뛰고 기존 DB 사용) ---")
    final_report_second_run = asyncio.run(_generate_page_4_future_report(test_query, perform_serper_search=False))
    print("\n--- 두 번째 실행 최종 보고서 ---")
