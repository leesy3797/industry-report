...
□ **[핵심 이슈 1 제목]**
    ○ 2021년 6월 cj대한통운은 [육하원칙에 따른 간결한 설명].
    - 이는 [관련 배경 또는 원인]으로 인해 발생했으며 [주요 내용 요약].
      • 특히 **[강조할 핵심 내용]**이 주목됨.

□ **[핵심 이슈 2 제목]**
    ○ cj대한통운은 해당 월에 [두 번째 핵심 이슈에 대한 간결한 설명].
...
2.  **본문 (Annual Key Issues & Trends):**
    ...
    * **각 이슈별 분량은 최대 4~5문장 이내**로 제한합니다.
    * 객관적인 사실만을 종합(반드시 정확한 수치, 용어, 인물을 언급하세요.)하고, 불필요한 서론/결론 없이 핵심 정보만 전달합니다.
    * 반드시 구체화된 내용(정량적 수치, 정확한 용어 등)으로 신뢰성을 확보하고 구체화된 언어로 작성합니다.
    ...
    * 중요한 부분은 `**볼드체**`로 강조합니다.
    * **각 연도별 보고서의 총 분량은 A4 용지 기준으로 1/4 ~ 1/3 페이지 (약 200-250자) 내외를 목표로 합니다.** 이를 통해 여러 연도 통합 시 2-3페이지를 맞출 수 있습니다.
...
□ **(#1) [핵심 이슈 1 제목]**
    ○ {year}년 {company}은 [해당 이슈가 연간 어떻게 전개되었는지 핵심적인 2~3문장으로 요약].
...
[지시사항]
당신은 **{company} 미래전략실의 신사업개발 전문가**입니다. 아래 제공되는 [검색된 웹 데이터]를 {company}의 **현재 핵심 역량 및 사업적 맥락**에서 철저히 분석하고, 이를 기반으로 **혁신적인 미래 비전 보고서**를 작성해야 합니다.

이 보고서는 어떠한 기업이든 적용 가능한 구체성과 깊이를 담아야 하며, 글로벌 탑티어 전략 컨설팅 펌의 스타일을 반영하여 **깊이 있는 통찰력과 구체적인 실행 방안**을 제시해야 합니다.
...
2.  **시간 흐름별 비전 (5년 후, 10년 후, 20년 후):**
    ...
    * **전략적 관점:** 단순히 현황을 나열하는 것이 아니라, **검색된 데이터와 {company}의 역량을 엮어 시장을 선점할 수 있는 독점적 전략과 신사업을 창조**하는 통찰력을 보여주십시오. 각 시점별 비전은 다음 시점 비전으로 자연스럽게 연결되는 논리적 흐름을 제시합니다.
    * **분량 목표:** 전체 보고서는 1,800~2,000자 내외의 상세 보고서로 작성되어야 합니다. (각 시간 구간별 약 600~700자 할당)
...
3.  **비전 내용의 구체화 (Actionable & Visionary Insights):**
    ...
    * 특정 파트너사나 협력 기업을 직접적으로 언급하는 대신, **'주요 파트너사', '핵심 기술 기업'**과 같이 포괄적인 표현을 사용하여 범용성을 확보합니다.
    * '글로벌 시장'과 같은 광범위한 표현을 사용할 경우, **'전략적 핵심 거점' 또는 '성장이 예상되는 특정 지역'**과 같이 구체적인 방향성을 제시하여 전략의 명확성을 높입니다.

4.  **보고서 형식:**
    * '청와대 비서실 보고서 표준서식'의 항목 부호 체계와 들여쓰기를 준수합니다.
    * **□ (들여쓰기 없음):** 각 시간 구간(5년 후, 10년 후, 20년 후)의 핵심 키워드나 소제목을 다는 데 사용됩니다. (ex. 5년 후: 핵심 키워드 및 소제목)
